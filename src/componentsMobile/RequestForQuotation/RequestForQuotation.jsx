...
import AddItemsToRFQ from './AddItemsToRFQ';
import ItemCard from './ItemCard';
import DatePickerModal from './DatePickerModal';
import DeleteConfirmModal from './DeleteConfirmModal';
import DuplicateRFQConfirmModal from './DuplicateRFQConfirmModal';
...
const RequestForQuotation = ({ user, onLogout }) => {
  const navigate = useNavigate();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [currentPage, setCurrentPage] = useState('request-for-quotation');
  ...
  const proceedWithPOGeneration = async () => {
    ...
    try {
      ...
      const baseUrl = "http://localhost:8082/api/rfq";
