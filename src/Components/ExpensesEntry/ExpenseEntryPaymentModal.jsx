...
import { isPaymentModeRequiringBankRegisterLog } from '../../utils/bankRegisterLogBeforeWeeklyBill';

const PAYMENT_MODAL_SELECT_Z_INDEX = 10001;

const isOnlinePaymentMode = (paymentMode) => isPaymentModeRequiringBankRegisterLog(paymentMode);
...
const ExpenseEntryPaymentModal = ({
  isOpen,
  onClose,
  onSubmit,
  isSubmitting = false,
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-[10000]">
      <div className="bg-white text-left rounded-xl p-6 w-[800px] max-h-[90vh] overflow-y-auto flex flex-col relative z-[10000]">
            ...
            <div className="border-2 border-[#BF9853] border-opacity-25 w-full rounded-lg p-4">
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <CustomDateField value={date || ''} onChange={() => {}} disabled placeholder="Date" />
                </div>
                <div>
            ...
            {isOnlinePaymentMode(paymentMode) && (
              <div className="border-2 border-[#BF9853] border-opacity-25 w-full rounded-lg p-4">
                <div className="space-y-4">
                  {String(paymentMode || '').trim().toLowerCase() === 'cheque' && (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setPaymentModalData((prev) => ({ ...prev, chequeNo: e.target.value }))
                          }
        ...
        <div className="flex justify-end gap-3 mt-6 p-4 bg-white border-t">
          <button type="button" onClick={onClose} className="px-4 py-2 border border-[#BF9853] text-[#BF9853] rounded-lg">
            Cancel
          </button>
          <button type="button" onClick={onSubmit} disabled={isSubmitting} className="px-4 py-2 bg-[#BF9853] text-white rounded-lg disabled:bg-gray-400">
            {isSubmitting ? 'Saving...' : 'Submit'}
          </button>
        </div>
        <button type="button" onClick={onClose} className="absolute top-3 right-4 text-xl font-bold text-gray-500 hover:text-black">
          ×
        </button>
      </div>
    </div>
...
export default ExpenseEntryPaymentModal;
